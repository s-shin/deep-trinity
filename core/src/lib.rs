...
//             debug_println!(self, "search_all: {:?} ({}, {})", fp.placement.orientation, fp.placement.pos.0, fp.placement.pos.1);
//             if self.all_checked.contains(&fp.placement) {
//                 debug_println!(self, "=> already checked.");
//                 return None;
//             }
//             self.last_checked = Some(fp.placement.clone());
//             let ok = self.check_sheet.insert(fp.placement);
//             debug_assert!(ok);
//
//             let mut fp = fp.clone();
//             if self.dsts.remove(&fp.placement) {
//                 debug_println!(self, "=> found!");
//                 return Some(MoveLog::new(fp.placement.pos));
//             }
//
...
//                     self.depth += 1;
//                     let search_fp = FallingPiece::new(fp.piece, fp.placement);
